...
def getUUID(comm_name):
    ...
    result = CommunitiesListService.filterListByName(
        allComm, '.*' + comm_name + '.*')
    result = str(result)
    ...
    if (counter < 1):
        ...
        while index < len(result):
            ...
            comm_id.append(
                result[result.find('uuid=', index) + 5:result.find('uuid=', index) + 41])
            comm_name.append(
                result[result.find('name=', index) + 5:result.find('uuid=', index) - 2])
            acl.append(result[result.find('type=', index) +
                              5:result.find('name=', index) - 2])
    ...
    if state == 'X':
        ...
        if parent > 0:
            ...
            while goBack != ('TRUE'):
                ...
                comm_id_parent, parent2, comm_full_name_parent, acl_parent, state = getUUID(
                    comm_name_parent)
                if state == 0:
                    continue
                if parent2 > 0:
                    ...
                    decision = raw_input('\nDo you really want to move the subcommunity ' +
                                         comm_fullname + ' into ' + comm_full_name_parent + ' (y/n) ? ')

                    if decision == 'y':
                        CommunitiesService.moveCommunityToSubcommunity(
